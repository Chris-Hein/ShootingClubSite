...
function formulate_v($atts, &$v, &$form = '', &$amount = '', &$id = '', &$stock = '', &$labels = '') {
	...
	*/
    ...
    $v['combine'] = $v['couponapplied'] = $v['couponget'] =$v['maths'] = $v['explodepay'] =  $v['explode'] = $v['recurring'] = $v['termschecked']  = $v['consent'] = '';
	...
	if ($qpp['use_multiples']) {
		...
		for ($i=1;$i<=9;$i++) {
            ...
            if (isset($d['qtyproduct'.$i]) && $d['qtyproduct'.$i] == 'checked') $d['qtyproduct'.$i] = 1;  //converts checked to 1
...
function qpp_display_form($values, $errors, $id, $attr = '') {
	...
	*/
        ...
        switch ( $name ) {
            ...
            if (!$qpp['use_multiples']) {
                if (!$values['setref']) {
					...
					$content .= qpp_nice_label('reference'.$id,'reference','text',$qpp['inputreference'],$label,$required.$errors['reference'],$values['reference']);
                } else {
                    if ($values['combine']) {
                        $checked = 'checked';
            ...
            if ($qpp['use_stock']) {
                ...
                if ($qpp['fixedstock'] || isset($_REQUEST["item"])) {
					...
					$content .= qpp_nice_label('itemnumber'.$id,'itemnumber','text',$qpp['stocklabel'],$label,$requiredstock.$errors['use_stock'],$values['stock']);
                }
            }
            break;			
                ...
                if ($qpp['use_slider'] && !$values['combine']) {
                    ...
                    if (!$values['combine']) {
                        if (!$values['setpay']){
							...
							// qppamount'.$t.'
							$content .= qpp_nice_label('amount'.$id,'amount','text',$qpp['inputamount'],$label,$required.$errors['amount'],$values['amount']);
                        } else {
            ...
            if ($qpp['usecoupon'] && ($values['couponapplied'] != 'checked')) {
				...
				$content .= qpp_nice_label('coupon'.$id,'couponblurb','text',$qpp['couponblurb'],$label,'',$values['couponblurb']);
            ...
            if ($qpp['userecurring'] && !$qpp['use_multiples']) {
                ...
                if ($qpp['variablerecurring']) $content .= '<input type="text" style=" '.$errors['srt'].'width:3em;margin-left:5px" id="srt'.$t.'" label="srt" name="srt" value="' . $values['srt'] . '" onfocus="qppclear(this, \'' . $values['srt'] . '\')" onblur="qpprecall(this, \'' . $values['srt'] . '\')" /> '. $qpp['every'].' '.$qpp[$recurringperiod];
            ...
            if ($qpp['useaddress']) {
                ...
                foreach($arr as $item)
                    if ($address[$item]) {
						...
						$content .= qpp_nice_label($item.$id,$item,'text',$address[$item],$label,$required.$errors[$item],$values[$item]);
            ...
            if ($qpp['useemail'] && !$qpp['useaddress']) {
				...
				$content .= qpp_nice_label('email'.$id,'email','text',$qpp['emailblurb'],$label,$requiredstock.$errors['email'],$values['email']);
            ...
            if ($qpp['use_message']) {
				...
				$content .= qpp_nice_label('yourmessage'.$id,'yourmessage','textarea',$qpp['messagelabel'],$label,$requiredstock.$errors['use_message'],$values['yourmessage']);
            ...
            if ($qpp['use_datepicker']) {
				...
				$content .= qpp_nice_label('qppdate'.$id,'datepicker','text',$qpp['datepickerlabel'],$label,$required,$values['datepicker']);
                // $content .= '<input type="text" id="qppdate" name="datepicker" '.$required.'value="' . $values['datepicker'] . '" onfocus="qppclear(this, \'' . $values['datepicker'] . '\')" onblur="qpprecall(this, \'' . $values['datepicker'] . '\')"/>';
				$content .= '<script type="text/javascript">jQuery(document).ready(function() {jQuery(\'\#qppdate'.$id.'\').datepicker({dateFormat : \'dd M yy\'});});</script>';
            }
            break;
            ...
            if ($qpp['use_cf']) {
                ...
                $content .= qpp_nice_label('cf'.$id,'cf','text',$qpp['cflabel'],$label,$requiredcf.$errors['use_cf'],$values['cf']);
            }
            break;
            ...
            if ($qpp['use_consent']) {
                ...
                <input type="checkbox" name="consent" value="checked" ' . $values['consent'] . '>
                &nbsp;'.$qpp['consentlabel'].'</p>';
            }
            break;
    ...
    if ($qppkey['authorised'] && $list['enable']) {
        $content .= '<p><input type="checkbox" name="mailchimp" value="checked" '.$values['mailchimp'].'>&nbsp;'.$list['mailchimpoptin'].'</p>';
    }
...
function qpp_messagetable ($id,$email) {
    ...
    foreach (explode( ',',$options['sort']) as $name) {
        ...
        switch ( $name ) {
            ...
            case 'field2': if (!$options['use_multiples']) $dashboard .= '<th style="text-align:left">'.$options['quantitylabel'].'</th>';break;
            ...
            case 'field21': if ($options['use_cf']) $dashboard .= '<th style="text-align:left:max-width:20%;">'.$options['cflabel'].'</th>';break;
            case 'field22': if ($options['use_consent']) $dashboard .= '<th style="text-align:left:max-width:20%;">Consent</th>';break;
        }
    }
...
function qpp_messagecontent ($id,$value,$options,$c,$messageoptions,$address,$arr,$i,$email) {
    ...
    foreach (explode( ',',$options['sort']) as $name) {
        ...
        switch ( $name ) {
            ...
            case 'field2': if (!$options['use_multiples']) $content .= '<td>'.$value['field2'].'</td>';break;
            ...
            case 'field21': if ($options['use_cf']) {
                if ($options['cflabel'] == $value['field21']) $value['field21']='';
                $content .= '<td>'.$value['field21'].'</td>';}break;
            case 'field22': if ($options['use_consent']) {
                if ($options['consentlabel'] == $value['field22']) $value['field22']='';
                $content .= '<td>'.$value['field22'].'</td>';}break;
        }
    }
    if ($messageoptions['showaddress']) {
...
function qpp_send_confirmation ($values,$form,$amounttopay) {
    $qpp_setup = qpp_get_stored_setup();
    $qpp = qpp_get_stored_options($form);
    $address = qpp_get_stored_address($form);
    $send = qpp_get_stored_send($form);
    ...
    $auto['fromemail'] = empty($auto['fromemail']) ? get_bloginfo('name')  : $qpp_setup['email'] ;
    $confirmemail = empty($send['confirmemail']) ? get_bloginfo('admin_email') : $send['confirmemail'];
...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
    ...
    if ($qpp['use_multiples']) {
        ...
        foreach ($values['items'] as $k => $item) {
            if ($item['quantity']) $multiple .= '<tr><td>'.$item['item_name'].'</td><td>'.$item['quantity'].'</td></tr>';
        }
        $multiple .= '</table>';
        $details .= $multiple;
    } else {
        $details .= '<p>'.$ref.$rcolon.$values['reference'].'</p>
        <p>'.$qpp['quantitylabel'].': '.$values['quantity'].'</p>';
    }
	...
	*/
	...
	if ($qpp['use_multiples']) {
		...
		if (isset($_POST['processing_type'])) {
			...
			$processing = $qpp['processfixed']/* * $qty */;
		...
		*/
		...
		if (isset($_POST['postage_type'])) {
			...
			$postage = (float) $qpp['postagefixed'];
		...
		if (isset($_POST['processing_type'])) {
			...
			$processing = (float) $qpp['processfixed'];
...
function qpp_nice_label($id,$name,$type,$label,$labelType,$error,$value) {
	...
	if ($type == 'text') {
		switch ($labelType) {
			case 0:
				...
				$returning .= '<input type="text" id="'.$id.'" name="'.$name.'" value="'.$v.'" '.$error.' />';
			break;
			case 1:
				$v = (($value == $label)? '':$value);
			...
			case 2:
				...
				$returning .= '<input type="text" id="'.$id.'" name="'.$name.'" value="'.$value.'" '.$error.' onfocus="qppclear(this, \'' . $value . '\')" onblur="qpprecall(this, \'' . $value . '\')"/>';
			break;
			case 3:
				$v = (($value == $label)? '':$value);
