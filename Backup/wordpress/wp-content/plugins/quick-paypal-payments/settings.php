...
function qpp_setup ($id) {
    ...
    if( isset( $_POST['Submit']) && check_admin_referer("save_qpp")) {
        ...
        $qpp_setup['nostore'] = $_POST['nostore'];
    ...
    <p><input style="margin:0; padding:0; border: none"type="checkbox" name="nostore" ' . $qpp_setup['nostore'] . ' value="checked"> Do not store messages in the database</p>
...
function qpp_styles($id) {
    ...
    $round=$rounded=$color=$white=$square=$theme=$submitrandom=$submitmiddle=$submitpixel=$submitright=false;    
    $qpp_setup = qpp_get_stored_setup();
    $id=$qpp_setup['current'];
...
function qpp_autoresponce_page($id) {
    ...
    <p class="description">If you want to receive notificationmessages use the option on the <a href="?page=quick-paypal-payments/settings.php&tab=reply">Send Options</a> tab.</p>
    <form method="post" action="">
...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
    ...
    if (!$qppkey['authorised']) {
        ...
        <p>If you want this feature on your site you need a <a href="https://www.paypal.com/uk/webapps/mpp/merchant" target="_blank">PayPal Business Account</a> and a one-off payment of $20. If you already have a PayPal Business Account click on the button below to upgrade.</p>
    ...
    $content .= '</form>
        ...
        $sandbox = $qpp_setup['sandbox'] ? 'Sandbox ' : '';
        $content = '<div class="qpp-settings"><div class="qpp-options">
...
function qpp_generate_csv() {
    ...
    if(isset($_POST['download_qpp_csv'])) {
        ...
        if ($ipn['ipn']) array_push($headerrow, 'Paid');
